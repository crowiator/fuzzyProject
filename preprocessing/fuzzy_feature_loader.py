...
from preprocessing.feature_extraction import extract_features_for_fuzzy, extract_ecg_features
...
def load_or_extract_fuzzy_features2():

    all_beat_counts_by_record = {}

    # Ak cache existuje, načítame predspracované features a labely
    if FUZZY_FEATURE_CACHE_CAMFEA.exists():
        print(f"Loading features from cache: {FUZZY_FEATURE_CACHE_CAMFEA  }")
        data = np.load(FUZZY_FEATURE_CACHE, allow_pickle=True)
        return data["features"].tolist(), data["labels"].tolist(), all_beat_counts_by_record
    ...
    for record in RECORD_NAMES:
        try:
            ...
            # Detegované R-vlny (od 1. pozície, nie od 0) a extrakica príznakov
            detected_r_peaks = r_peak_positions
            features = extract_ecg_features(signal_for_amplitude, detected_r_peaks, fs)
            print(features[:3])
    ...
    np.savez(FUZZY_FEATURE_CACHE_CAMFEA , features=np.array(all_features, dtype=object), labels=np.array(all_labels))
    print(f"Dataset is saved into : {FUZZY_FEATURE_CACHE_CAMFEA }")
